...
///
// dumpBuffer(msg) - dump the contents of the BufferSet
//
// @param msg   description of the buffer
///
void BufferSet::dumpBuffer( const char *msg ) {
    cout << "Dumping buffer " << msg << " (";
    if( !bufferInit ) {
        cout << "not ";
    }
...
//
// @param dump  the Canvas we'll use for drawing
// @param C     the Canvas we'll use for drawing
///
void BufferSet::createBuffers( bool dump, Canvas &C ) {
    ...
    // Do we want to dump the data?
    if( !dump ) {
        return;
    }

    // Yes!  Start with the element count
    cout << "Elements " << numElements << endl;

    // dump the vertices - four values per vertex
    cout << "Vertices" << endl;
    float *fp = points;
    for( int i = 0; i < numElements; ++i ) {
        cout << " [" << setw(4) << i << "]: ";
        cout << " " << setw(10) << setprecision(6) << *fp++ << "f,";
        cout << " " << setw(10) << setprecision(6) << *fp++ << "f,";
        cout << " " << setw(10) << setprecision(6) << *fp++ << "f,";
        cout << " " << setw(10) << setprecision(6) << *fp++ << "f," << endl;
    }
    ...
    // dump the color data if there is any
    if( colors ) {
        cout << "Color" << endl;
        fp = colors;
        for( int i = 0; i < numElements; ++i ) {
            cout << " [" << setw(4) << i << "]: ";
            cout << " " << setw(8) << setprecision(6) << *fp++ << "f,";
            cout << " " << setw(8) << setprecision(6) << *fp++ << "f,";
            cout << " " << setw(8) << setprecision(6) << *fp++ << "f,";
            cout << " " << setw(8) << setprecision(6) << *fp++ << "f," << endl;
        }
    }

    // dump the normal data if there is any
    if( normals ) {
        cout << "Normals" << endl;
        fp = normals;
        for( int i = 0; i < numElements; ++i ) {
    ...
    // dump the UV data if there is any
    if( uv ) {
...
void BufferSet::selectBuffers( GLuint program,
    ...
    GLint loc = glGetAttribLocation( program , vp );
    ...
    if( vc != NULL ) {
        loc = glGetAttribLocation( program, vc );
        glEnableVertexAttribArray( loc );
        glVertexAttribPointer( loc, 4, GL_FLOAT, GL_FALSE, 0,
    ...
    if( vt != NULL ) {
        loc = glGetAttribLocation( program, vt );
	// prevent some runtime messages if vTexCoords isn't actually used
        if( loc >= 0 ) {
            glEnableVertexAttribArray( loc );
