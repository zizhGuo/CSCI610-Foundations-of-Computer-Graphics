...
void BufferSet::selectBuffers( GLuint program,
    ...
    GLint vPosition = getAttribLoc( program , vp );
    ...
    if( vc != NULL ) {
        GLint vColor = getAttribLoc( program, vc );
    ...
    if( vn != NULL ) {
        GLint vNormal = getAttribLoc( program, vn );
    ...
    if( vt != NULL ) {
        GLint vTexCoord = getAttribLoc( program, vt );
